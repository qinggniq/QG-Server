...
TcpConnection::TcpConnection(event_loop_pt el, std::unique_ptr<Socket> socket,
  ...
  LOG(INFO) << "accpeter_->incConnection();";
  event_handler_->setReadCallBack(std::bind(&TcpConnection::handleRead, this));
  LOG(INFO) << "event_handler_->setWriteCallBack(";
...
void TcpConnection::handleRead() {
  ...
  while ((n = qg::read(socket_->sfd(), buf,
  ...
  if (cnt > 0) {
    // 这里不能再放到pendingfunctor里面再去执行了，因为
...
void TcpConnection::writeInLoop(const buf_t &data) {
  // 如果连接没有注册写事件回调，那么就直接写
  LOG(INFO) << "write data : " << data;

  if (disconnected_) {
    LOG(INFO) << "connection " << socket_->sfd() << " closed";
    return;
  }
  int len = data.size();
  ...
  if (canWriteDirect) {
    ...
    if (nwrite > 0) {
      if (nwrite == len && sendfile_fd_ < 0) {
        event_loop_->addFunctor(std::bind(write_complete_call_back_,
                                          shared_from_this(), write_buf_));
      }
      if (disconnected_) {
        handleClose();
        return;
...
void TcpConnection::write(const buf_t &data) {
  event_loop_->addFunctor(std::bind(&TcpConnection::writeInLoop, this, data));
}
  ...
  if (nwrite >= 0) {
    ...
    if (write_buf_.empty()) {
      ...
      event_loop_->addFunctor(
          std::bind(write_complete_call_back_, shared_from_this(), write_buf_));
    }
...
void TcpConnection::handleClose() {
  ...
  // handleClose被调用的时候肯定在ioloop中，可以安全的disableAll，防止在baseLoop等待close的时候又来了新的事件。
  ...
  if (connection_close_call_back_) {
    ...
    connection_close_call_back_(objectGurd);
  }
}

void TcpConnection::closeConnection() {
  LOG(INFO) << "closeConnection() : " << socket_->sfd() << " at " << this;
  disconnected_ = true;
