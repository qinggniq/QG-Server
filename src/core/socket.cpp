...
void Socket::makePortReuseable() {
  qg_int enable = 1;
  if (setsockopt(sfd_, SOL_SOCKET, SO_REUSEPORT, &enable, static_cast<socklen_t >(sizeof(qg_int))) < 0);
  //TODO (qinggniq) log
...
void Socket::makeKeepAlive() {
  qg_int enable = 1;
  if (setsockopt(sfd_, SOL_SOCKET, SO_KEEPALIVE, &enable, static_cast<socklen_t>(sizeof(qg_int))) < 0);
  //TODO (qinggniq) log
}

void Socket::makeCork() {
  qg_int optval = 1;
  if (setsockopt(sfd_, SOL_SOCKET, QGIO_NOPUSH, &optval, sizeof(optval)) == -1) {
	//TODO log
    //close(sfd);
  }
}
...
void Socket::bindAndListen(qg_int port) {
  ...
  qg_int ret = ::bind(sfd_, (struct sockaddr *)&serv_addr, static_cast<socklen_t>(sizeof(struct sockaddr)));
  if (ret != 0) {
    //LOG
    switch (errno) {
      case EADDRINUSE:
...
qg_fd_t Socket::accept() {
  ...
  qg_fd_t new_fd = ::accept(sfd_, &cli_addr, &addr_len);
