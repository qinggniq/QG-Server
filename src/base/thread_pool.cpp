...
namespace qg {
template <typename T>
ThreadPool<T>::ThreadPool(qg_size_t pool_size, qg_size_t queue_size)
    : pool_size_(pool_size), max_req_size_(queue_size) {
...
template <typename T> void ThreadPool<T>::addTask(T *t) {
  pthread_mutex_lock(&mutex_);
  while (task_queue_.size() >= max_req_size_) {
    pthread_cond_wait(&no_full_, &mutex_);
  }
  task_queue_.push(t);
  pthread_cond_signal(&no_empty_);
  pthread_mutex_unlock(&mutex_);
}
template <typename T> void ThreadPool<T>::execTask() {
  while (true) {
...
template <typename T> void ThreadPool<T>::start() {
  ...
  for (int i = 0; i < pool_size_; ++i) {
    pthread_create(&threads[i], nullptr, this->execTask, nullptr);
